...
# Description:
# Add backward gradient for the origin and direction of rays into 3dgrt tracer
# The original version uses ray space coordinates for input, while this version uses world space.
...
class Tracer:
    class _Autograd(torch.autograd.Function):
        ...
        def backward(
            ...
            # print("ray_ori: ", torch.isnan(ray_ori_grd).any())
            # print("ray_dir: ", torch.isnan(ray_dir_grd).any())
            mog_pos_grd, mog_dns_grd, mog_rot_grd, mog_scl_grd, _ = torch.split(
                particle_density_grd, [3, 1, 4, 3, 1], dim=1
            )
            # if ctx.frame_id % 2 == 0:
            #     ray_ori_grd = torch.zeros_like(ray_ori_grd)
            #     ray_dir_grd = torch.zeros_like(ray_dir_grd)
            # else:
            # mog_pos_grd = torch.zeros_like(mog_pos_grd)
            # mog_rot_grd = torch.zeros_like(mog_rot_grd)
            # mog_scl_grd = torch.zeros_like(mog_scl_grd)
            # mog_dns_grd = torch.zeros_like(mog_dns_grd)
            # mog_sph_grd = torch.zeros_like(mog_sph_grd)
    ...
    def render(self, gaussians, gpu_batch: Batch, train=False, frame_id=0):
        ...
        return {
            ...
            "render_time_ms": self.frame_timer.timing() if self.frame_timer is not None else 0.0,
