...
OptixTracer::traceBwd(uint32_t frameNumber,
    ...
    torch::Tensor rayOriginGrad = torch::empty({rayOri.size(0), rayOri.size(1), rayOri.size(2), 3}, opts);
    torch::Tensor rayDirectionGrad = torch::empty({rayOri.size(0), rayOri.size(1), rayOri.size(2), 3}, opts);
    ...
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>(particleDensityGrad, particleRadianceGrad, rayOriginGrad, rayDirectionGrad);
}
